...
/**
 * REST controller for managing {@link com.vfin.theconnect.domain.Image}.
 */
@RestController
@RequestMapping("/api")
public class ImageResource {
     ...
     */
    @GetMapping("/images")
    public ResponseEntity<List<ImageDTO>> getAllImages(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("accountinfo-is-null".equals(filter)) {
            log.debug("REST request to get all Images where accountInfo is null");
            return new ResponseEntity<>(imageService.findAllWhereAccountInfoIsNull(),
                    HttpStatus.OK);
        }
